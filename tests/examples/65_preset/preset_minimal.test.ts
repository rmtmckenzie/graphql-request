...
test(`preset_minimal`, async () => {
  const exampleResult = await runExample(`./examples/65_preset/preset_minimal.ts`)
  ...
  await expect(exampleResultMaybeEncoded).toMatchFileSnapshot(
    `../../../examples/__outputs__/65_preset/preset_minimal.output.txt`,
  )
})
